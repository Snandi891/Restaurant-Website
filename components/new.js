    ...
    <>
      ...
      <main className="bg-white text-gray-800">
        ...
        <section className="min-h-[75vh] md:h-screen bg-[url('/restaurant.jpg')] bg-cover bg-center flex items-center justify-end text-white relative rounded-b-[80px] overflow-hidden">
          ...
          <section className="relative min-h-screen flex flex-col md:flex-row items-center justify-center md:justify-between px-6 md:px-66">
            {/* Left Glow Image */}
            <motion.div
              initial={{ scale: 0.8, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              transition={{ duration: 1 }}
              className="relative mt-10 md:mt-0 order-2 md:order-1 flex justify-start md:justify-start"
            >
              <div className="relative w-56 h-56 sm:w-72 sm:h-72 md:w-126 md:h-126 rounded-full overflow-hidden shadow-[0_0_60px_#facc15] ml-[-180px] md:ml-[-250px]">
                <img
                  src="/restaurant.jpg" // replace with your food image
                  alt="Signature Dish"
                  className="w-full h-full object-cover rounded-full"
                />
                {/* Glow Pulse */}
                <div className="absolute inset-0 rounded-full bg-yellow-500 opacity-20 animate-ping"></div>
              </div>
            </motion.div>
            ...
            <motion.div
              ...
              transition={{ duration: 1 }}
              className="text-center md:text-right max-w-xl order-1 md:order-2"
            >
              <h1 className="text-4xl md:text-6xl font-extrabold leading-tight">
                Experience{" "}
                <span className="text-yellow-500">Authentic Flavors</span>
              </h1>
              <p className="mt-4 text-lg text-gray-300">
                A premium dining experience where tradition meets taste. Join us
                for unforgettable meals crafted with passion.
              </p>
              <div className="mt-6 flex gap-4 justify-center md:justify-end">
                <Link
                  href="#menu"
                  className="px-6 py-3 rounded-full bg-yellow-500 text-black font-semibold hover:bg-yellow-400 shadow-lg"
                >
                  Explore Menu
                </Link>
                <button
